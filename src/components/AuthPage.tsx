...
const AuthPage = () => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (error) {
          // Log failure
          try { await logAction('unknown', email, 'login_failure', `Login failed: ${error.message}`); } catch (e) { /* ignore */ }
          throw error;
        }

        // Successful login - attempt to log with user id if available
        try {
          const userId = data?.session?.user?.id || 'unknown';
          await logAction(userId, email, 'login_success', 'User signed in');
        } catch (e) {
          console.warn('Failed to write login_success to audit log', e);
        }
      } else {
        ...
        if (error) {
          try { await logAction('unknown', email, 'signup_failure', `Signup failed: ${error.message}`); } catch (e) { /* ignore */ }
          throw error;
        }

        // Signup initiated successfully (may require email confirmation)
        try {
          const userId = data?.user?.id || 'unknown';
          await logAction(userId, email, 'signup_success', 'User signed up (may require email confirmation)');
        } catch (e) {
          console.warn('Failed to write signup_success to audit log', e);
        }
      ...
      // Also log the error generically if not already logged
      try {
        await logAction('unknown', email, 'auth_error', `Auth action error: ${err?.message || String(err)}`);
      } catch (e) {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (error) {
        try { await logAction('unknown', undefined, 'oauth_login_failure', `Google OAuth failed: ${error.message}`); } catch (e) {}
        throw error;
      }

      // For OAuth flows, we can log the attempt; completion will be handled by onAuthStateChange
      try { await logAction('unknown', undefined, 'oauth_login_attempt', 'User initiated Google OAuth login'); } catch (e) {}
    } catch (err: any) {
